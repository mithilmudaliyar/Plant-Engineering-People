      ...
      <Container>
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-black text-[#0C1B33]">Contact Inquiries</h1>
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden self-start">
              <button
                ...
                className={`px-4 py-2 text-sm font-bold transition-colors ${filter === "unread" ? "bg-[#0C1B33] text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}
              >
                Unread{unreadCount > 0 && <span className="ml-1.5 bg-[#d41f3d] text-white rounded-full px-1.5 py-0.5 text-[10px]">{unreadCount}</span>}
              </button>
              <button
                onClick={() => setFilter("all")}
                className={`px-4 py-2 text-sm font-bold transition-colors ${filter === "all" ? "bg-[#0C1B33] text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}
              >
                All
              </button>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 surface">
              ...
              {filter === "unread" && (
                <button onClick={() => setFilter("all")} className="mt-3 text-sm font-bold text-[#0C1B33] hover:underline">
                  View all
                </button>
              )}
            </div>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        {!inquiry.read && <span className="inline-block h-2 w-2 rounded-full bg-[#d41f3d] shrink-0" />}
                        <p className="font-bold text-[#0C1B33] truncate">{inquiry.name}</p>
                        {inquiry.company && <span className="text-xs text-slate-400">· {inquiry.company}</span>}
                      </div>
                    ...
                    <div className="px-6 pb-6 border-t border-slate-100 pt-4">
                      <div className="grid sm:grid-cols-3 gap-4 mb-4 text-sm bg-slate-50 rounded-lg p-4">
                        <div>
                          ...
                          <p className="font-semibold text-[#0C1B33]">{inquiry.name}</p>
                        </div>
                        {inquiry.company && (
                          <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Company</p>
                            <p className="font-semibold text-[#0C1B33]">{inquiry.company}</p>
                          </div>
                        )}
                        <div>
                      ...
                      <div className="flex items-center gap-3">
                        <a
                          ...
                          className="inline-flex items-center gap-2 bg-[#0C1B33] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#1a3a52] transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="text-sm text-slate-500 hover:text-[#0C1B33] font-semibold transition-colors"
                        >
